...
import dao.GetRoomByNumberDatabase;
import resource.*;
import rest.*;
...
public final class RestManagerServlet extends AbstractServlet {
	...
	private boolean checkMethodMediaType(final HttpServletRequest req, final HttpServletResponse res)
		...
		switch(method) {
			...
			case "POST": break; // actually the post should require in input the json. When done so remove the break;
			case "PUT":
				if(contentType == null) {
	...
	private boolean processBooking(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		try{
			if (tokens.length == 6 && tokens[4].equals("customer")){
			...
			} else if (tokens.length==5 && tokens[4].equals("insert")) {
				// the request URI is: /booking/insert/{parameters}
				...
				UUID personId = (UUID) req.getSession(false).getAttribute("personId");
				String user = null;
				if (personId == null) {
					...
					user = (String) req.getSession(false).getAttribute("email");
				}

				// parsing checkin and checkout parameters as Timestamps
				String[] tk = checkin.split("T");
				checkin = tk[0]+" "+tk[1] +":00";
				tk = checkout.split("T");
				checkout = tk[0]+" "+tk[1] +":00";
				Timestamp cin = Timestamp.valueOf(checkin);
				Timestamp cout = Timestamp.valueOf(checkout);

				// parsing paymentMethod
				PaymentMethod pm = PaymentMethod.valueOf(paymentMethod.toUpperCase(Locale.ROOT).replace(' ', '_'));

				// search room and then compute totalAmount of payment
				Room room = new GetRoomByNumberDatabase(getDataSource().getConnection(), req.getParameter("room")).getRoomByNumber();
				double totAmount = room.getPrice() * (ChronoUnit.DAYS.between(cin.toLocalDateTime(), cout.toLocalDateTime()));

				Booking booking = new Booking(UUID.randomUUID(), personId, cin, cout, paymentId, requests);
				Payment payment = new Payment(paymentId, pm, totAmount, user);
	...
	private boolean processCustomer(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if (tokens.length<4 || !(tokens[3].equals("customer"))){
			return false;
		}

		try{
			if (tokens.length == 5 && tokens[4].equals("list")){
				// the request URI is: /customer/list

				CustomerRestResource crr = new CustomerRestResource(req, res, getDataSource().getConnection());
				switch (req.getMethod()) {
					case "GET" -> crr.listCustomers();
					default -> writeError(res, ErrorCode.METHOD_NOT_ALLOWED);
				}
			} else {
				// the request URI is: /customer/{name surname}

				CustomerRestResource crr = new CustomerRestResource(req, res, getDataSource().getConnection());
				switch (req.getMethod()) {
					case "GET" -> crr.getCustomersByName();
					default -> writeError(res, ErrorCode.METHOD_NOT_ALLOWED);
				}
			}
		} catch (NumberFormatException e){
	...
	private boolean processRoom(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if (tokens.length<4 || !(tokens[3].equals("room"))){
			return false;
		}

		try{
			if (tokens.length == 8 && tokens[4].equals("list-available")){
				// the request URI is: /room/list-available/{beds}/{checkin}/{checkout}

				//check if a UUID is inserted
				String beds = tokens[tokens.length - 3];
				String checkin = tokens[tokens.length - 2];
				String checkout = tokens[tokens.length - 1];
				...
				RoomRestResource rrr = new RoomRestResource(req, res, getDataSource().getConnection());
				switch (req.getMethod()) {
					case "GET" -> rrr.getAvailableRooms();
					default -> writeError(res, ErrorCode.METHOD_NOT_ALLOWED);
				}
			}
		} catch (NumberFormatException e){
	...
	private boolean processPayment(HttpServletRequest req, HttpServletResponse res) throws IOException {
		String op = req.getRequestURI();
		String[] tokens = op.split("/");

		if (tokens.length<4 || !(tokens[3].equals("payment"))){
			return false;
		}

		try{
			if (tokens.length == 6 && tokens[4].equals("customer")){
				// the request URI is: /payment/customer/{personid}

				//check if a UUID is inserted
				UUID id = UUID.fromString(tokens[tokens.length -1]);

				PaymentRestResource brr = new PaymentRestResource(req, res, getDataSource().getConnection());
				switch (req.getMethod()) {
					case "GET" -> brr.getPaymentListByCustomer();
					default -> writeError(res, ErrorCode.METHOD_NOT_ALLOWED);
				}
			} else if (tokens.length==6 && tokens[4].equals("booking")) {
				// the request URI is: /payment/booking/{bookingid}

				//check if a UUID is inserted
				UUID id = UUID.fromString(tokens[tokens.length -1]);
				PaymentRestResource brr = new PaymentRestResource(req, res, getDataSource().getConnection());
				switch (req.getMethod()) {
					case "GET" -> brr.getPaymentByBooking();
					default -> writeError(res, ErrorCode.METHOD_NOT_ALLOWED);
				}
			}
		} catch (NumberFormatException e){
