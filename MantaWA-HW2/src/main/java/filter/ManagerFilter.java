...
public class ManagerFilter extends AbstractFilter {
    ...
    final static Logger logger = LogManager.getLogger(ManagerFilter.class);
    ...
    public void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (loggedIn) {
            if (session.getAttribute("role") == null) {
                res.sendRedirect(unauthorizedPage);
                return;
            }
            if (session.getAttribute("role").equals("Hotel Manager")) {
                ...
                logger.info("user "+session.getAttribute("email")+
                        ...
                        " tried to access the manager page");
            ...
            // maybe has to be created a separate folder
        }
    }
